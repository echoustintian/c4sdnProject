<template>
    <div class="main">
        ...
        <el-table :data="data" style="width: 100%" :row-style="{ height: '45px' }"
            ...
            <el-table-column prop="position" label="宿舍楼" />
            <el-table-column prop="dormitoryNum" label="宿舍号" />
            <el-table-column prop="isIn" label="状态">
                <template #default="scope">
                    <el-tag :type="scope.row.isIn ? 'success' : 'info'" effect="dark">{{ scope.row.isIn ? '在寝' : '离去'
                    }}</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="lastEnterTime" label="进入时间" width="150" />
            <el-table-column prop="lastEnterTime" label="离去时间" width="150" />
